...
@onready var shooting_audio_player : AudioStreamPlayer2D = $ShootingAudio
...
func shoot(target_direction : Vector2):
	if not reload_timer.is_stopped():
		empty_clip.emit()
		return
	elif not interval_timer.is_stopped():
		return
	...
	randomize()
	shooting_audio_player.stream = Global.shooting_streams[randi() % len(Global.shooting_streams)]
